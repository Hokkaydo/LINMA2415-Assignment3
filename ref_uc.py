from pyomo.environ import ConcreteModel, Var, Objective, Constraint, NonNegativeReals, Binary, UnitInterval
import json
import sys
...
m.dg_index = [(g,s,t) for g in thermal_gens for s in gen_startup_categories[g] for t in time_periods] ##
...
print("solving")
cbc.solve(m, options={'MIPGap':1e-6}, tee=True)

# print number of unit committed
print('number of units committed:')
for t in time_periods:
    print('time period {}: {}'.format(t, sum(m.ug[g,t].value for g in thermal_gens)))
    
# print total load served
print('total load served:')
for t in time_periods:
    print('time period {}: {}'.format(t, sum(m.pg[g,t].value for g in thermal_gens) + sum(m.pw[w,t].value for w in renewable_gens)))
    
# print total renewable generation
print('total renewable generation:')
for t in time_periods:
    print('time period {}: {}'.format(t, sum(m.pw[w,t].value for w in renewable_gens)))